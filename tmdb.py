...
def get_movie_details(movie_id):
    return _make_request(f'movie/{movie_id}', {'append_to_response': 'credits'})

def get_show_details(show_id):
    return _make_request(f'tv/{show_id}', {'append_to_response': 'credits'})
...
def search(query, page=1):
    params = {'query': query, 'page': page, 'language':'ar'}
    data = _make_request('search/multi', params)
    return data.get('results', []), data.get('total_pages', 1)

def discover_movies(language='en', genre=None, page=1, sort_by='popularity.desc'):
...
def get_airing_today():
    data = _make_request('tv/airing_today', {'language':'ar'})
    return data.get('results', [])

def get_popular_by_lang(lang, media_type):
    data = _make_request(f'{media_type}/popular', {'language': lang, 'page': 1})
    return data.get('results', []), data.get('total_pages', 1)
...
def get_upcoming(page=1):
    data = _make_request('movie/upcoming', {'page': page})
    return data.get('results', []), data.get('total_pages', 1)

def get_details(id, media_type):
    """بتجيب تفاصيل فيلم او مسلسل حسب النوع"""
